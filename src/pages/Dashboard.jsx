...
export default function Dashboard() {
  ...
  useEffect(() => {
    // LOGIKA PER AKUN (Email sebagai kunci)
    ...
    journalsData.forEach((journal) => {
      ...
      } else if (kategori.includes("Enzym") || kategori.includes("enzyme")) {
  ...
  return (
    <div className="w-10/12 mx-auto p-5 bg-white min-h-screen">
      <div className="mb-6 flex justify-center text-5xl">
        {/* Trik potong email biar gak kepanjangan */}
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <button 
          ...
          className="bg-green-700 text-white px-4 py-2 rounded-lg font-bold text-sm shadow-md"
        >
          📜 Lihat Riwayat
        </button>
      </div>
      ...
      <div className="flex gap-2 mb-8">
        {fermentasiData.map((item) => (
          <div key={item.id} className="flex-1 border border-gray-300 rounded-lg overflow-hidden">
